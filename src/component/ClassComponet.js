...
class CameraFaceDetect extends Component {
  ...
  matcher = async () => {
    const {data} = await axios.get('http://localhost:5500/api/faceapi/faechAll')
    console.log(data)
    this.setState({faceDate:data})
    const faceMatcher = await createMatcher(this.state.faceDate);
